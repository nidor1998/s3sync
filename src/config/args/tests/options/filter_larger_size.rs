...
mod tests {
    ...
    fn with_custom_value() {
        ...
        assert_eq!(config.filter_config.larger_size.unwrap(), 65536);
    }

    #[test]
    fn with_custom_value_human_byte() {
        init_dummy_tracing_subscriber();
        ...
        assert_eq!(config.filter_config.larger_size.unwrap(), 8 * 1024 * 1024);
    }

    #[test]
    fn with_custom_value_over_5gib() {
        init_dummy_tracing_subscriber();
        ...
        assert_eq!(
            config.filter_config.larger_size.unwrap(),
            10 * 1024 * 1024 * 1024
        );
    }
